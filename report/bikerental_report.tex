...
\title{
    ...
    A Count Regression Analysis using Poisson and Negative Binomial GLM}
}
\author{
    William Chen
}
...
%-----------------------------------------------------------------------
\begin{abstract}
This report analyzes daily bike rental counts from the UCI Bike Sharing
...
count is 22. The final NB2 model achieves a McFadden $R^2$ of 0.761 and
a rolling-origin CV RMSE of 1{,}660, and identifies \textit{temp}
(IRR $= 4.51$), \textit{yr1} (IRR $= 1.62$), and \textit{weathersit3}
(IRR $= 0.48$) as the dominant predictors of daily rental demand.
\end{abstract}
...
demand is valuable for operators seeking to optimize fleet distribution,
...
effects, making it a well-suited problem for regression modeling.

This report analyzes daily bike rental counts using the UCI Bike Sharing
...
estimate, and McFadden $R^2$, with out-of-sample predictive performance
assessed via rolling-origin cross-validated RMSE and MAE. Coefficients
are interpreted via Incidence Rate Ratios (IRR $= e^{\hat{\beta}}$).
...
Poisson GLM is a generalized linear model suited for count data. It uses a
...
Coefficients are interpreted in the same way as Poisson via IRR. When the dispersion parameter $\alpha$ approaches zero, NB2 converges
to Poisson, making Poisson a special case of NB2.
...
Models are compared using both in-sample and out-of-sample metrics.
AIC and BIC balance goodness of fit against model complexity, with lower
values indicating a better model. The dispersion estimate (Residual
Deviance / degrees of freedom) verifies whether the equidispersion
assumption holds, with values close to 1 indicating a well-fitted model.
McFadden $R^2$ serves as a supplementary goodness-of-fit measure; however,
it should be interpreted with caution under Poisson when overdispersion is
present, as the inflated deviance can produce artificially high values.
Out-of-sample performance is assessed using CV RMSE and CV MAE, computed
via a rolling-origin strategy to respect the temporal structure of the data.
Binned residual plots provide a visual check of model fit by examining
whether average residuals fall within the 95\% confidence bands.
...
inherent difficulty of modeling extreme demand days.
...
Table~\ref{tab:nb2_irr} presents the full set of estimated coefficients,
expressed as Incidence Rate Ratios (IRR) with 95\% confidence intervals.

\begin{table}[h]
\centering
\caption{NB2 Model Coefficients — Incidence Rate Ratios (IRR)}
\label{tab:nb2_irr}
\begin{tabular}{lrrr}
\hline
\textbf{Variable} & \textbf{IRR} & \textbf{95\% CI Lower} & \textbf{95\% CI Upper} \\
\hline
(Intercept)    & 1593.166 & 1382.437 & 1837.075 \\
seasonSpring   & 1.389    & 1.301    & 1.483    \\
seasonSummer   & 1.249    & 1.147    & 1.360    \\
seasonFall     & 1.616    & 1.528    & 1.710    \\
yr1            & 1.619    & 1.561    & 1.678    \\
holiday1       & 0.809    & 0.724    & 0.907    \\
weekday1       & 1.067    & 0.996    & 1.143    \\
weekday2       & 1.086    & 1.016    & 1.161    \\
weekday3       & 1.075    & 1.006    & 1.150    \\
weekday4       & 1.099    & 1.028    & 1.175    \\
weekday5       & 1.122    & 1.049    & 1.200    \\
weekday6       & 1.094    & 1.023    & 1.169    \\
weathersit2    & 0.902    & 0.861    & 0.946    \\
weathersit3    & 0.483    & 0.427    & 0.547    \\
temp           & 4.508    & 3.755    & 5.412    \\
hum            & 0.744    & 0.622    & 0.891    \\
windspeed      & 0.506    & 0.393    & 0.652    \\
\hline
\end{tabular}
\begin{tablenotes}
\small
\item Reference categories: Winter (season), Year 0 (yr), Sunday (weekday),
      Clear weather (weathersit), Non-holiday (holiday).
      All continuous predictors are normalized to [0, 1].
\end{tablenotes}
\end{table}

The results highlight several key patterns. Temperature is the dominant 
predictor (IRR $= 4.508$), reflecting strong sensitivity of cycling demand 
to thermal comfort. Year-over-year growth (IRR $= 1.619$) and seasonal 
effects are also substantial, with autumn demand approximately 62\% higher 
than the Winter baseline. Adverse weather (IRR $= 0.483$) and high wind 
speed (IRR $= 0.506$) each suppress demand by roughly half. Among weekday 
indicators, Monday (\textit{weekday1}) is the only non-significant predictor, 
suggesting no meaningful difference from the Sunday baseline.


\subsection{Model Comparison}
Table~\ref{tab:model_compare} compares the two models across in-sample
and out-of-sample metrics.

\begin{table}[H]
    \centering
    \caption{Model Comparison. AIC, BIC, Dispersion, and McFadden $R^2$
    are in-sample; CV RMSE and CV MAE are out-of-sample, evaluated using
    a rolling-origin strategy with 5 expanding windows.}
    \label{tab:model_compare}
    \begin{tabular}{lrrrrrr}
        \toprule
        \textbf{Model} & \textbf{AIC} & \textbf{BIC} & \textbf{Dispersion} & \textbf{McFadden $R^2$} & \textbf{CV RMSE} & \textbf{CV MAE} \\
        \midrule
        Poisson & 137,806.50 & 137,884.61 & 182.596 & 0.805 & 1489.626 & 1143.205 \\
        NB2     &  12,193.16 &  12,275.86 &   1.046 & 0.761 & 1660.491 & 1287.878 \\
        \bottomrule
    \end{tabular}
\end{table}

NB2 outperforms Poisson on AIC, BIC, and dispersion, confirming that
overdispersion is resolved. The higher Poisson McFadden $R^2$ (0.805
vs.\ 0.761) is an artifact of overdispersion inflating the deviance-based
statistic.
...
from the Sunday baseline. Although Poisson achieves lower CV RMSE
(1489.6 vs.\ 1660.5), this reflects similar point estimates under the
same link function rather than superior model specification; NB2 is
preferred for its valid inference under overdispersion.
...
only two years of data, and the large \texttt{yr} coefficient may
reflect a growth phase specific to that period rather than a persistent
trend, limiting predictive validity for future years. Second, prediction
errors are likely larger on extreme weather days, as such events are
rare and poorly represented during training.

\paragraph{Methodological limitations.}
Two limitations concern model specification. First, the model excludes
interaction terms such as \texttt{temp:season}, which may capture more
nuanced demand dynamics and improve performance. Second, the model
assumes independence across observations; however, daily rental counts
exhibit inherent temporal structure. Residual autocorrelation was not
formally assessed in this analysis. Future work could examine ACF plots
or apply a Durbin-Watson test on model residuals, and consider
time-series extensions such as ARIMA-GLM or generalized additive models
with temporal smoothing terms.
